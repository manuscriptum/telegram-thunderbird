...
var MtpNetworkerFactory = new function () {
  ...
  MtpNetworker.prototype.setupMobileSleep = function () {
    ...
    $rootScope.$on('push_received', function () {
      // dump(dT(), 'push recieved', self.sleepAfter);
      if (self.sleepAfter) {
        self.sleepAfter = tsNow() + 30000;
        self.checkLongPoll();
      }
  ...
  MtpNetworker.prototype.wrapMtpCall = function (method, params, options) {
    ...
    if (Config.Modes.debug) {
      dump(dT(), 'MT call', method, params, messageID, seqNo);
    }
  ...
  MtpNetworker.prototype.wrapMtpMessage = function (object, options) {
    ...
    if (Config.Modes.debug) {
      dump(dT(), 'MT message', object, messageID, seqNo);
    }
  ...
  MtpNetworker.prototype.wrapApiCall = function (method, params, options) {
    ...
    if (Config.Modes.debug) {
      dump(dT(), 'Api call', method, params, messageID, seqNo, options);
    } else {
      dump(dT(), 'Api call', method);
    }
  ...
  MtpNetworker.prototype.checkLongPoll = function(force) {
    ...
    // dump('Check lp', this.longPollPending, tsNow(), this.dcID, isClean);
  ...
  MtpNetworker.prototype.sendLongPoll = function() {
    ...
    // dump('Set lp', this.longPollPending, tsNow());
  ...
  MtpNetworker.prototype.pushResend = function(messageID, delay) {
    ...
    // dump('Resend due', messageID, this.pendingMessages);
  ...
  MtpNetworker.prototype.checkConnection = function(event) {
    ...
    dump(dT(), 'Check connection', event);
    ...
    this.sendEncryptedRequest(pingMessage, {timeout: 15000}).then(function (result) {
      ...
      dump(dT(), 'Delay ', self.checkConnectionPeriod * 1000);
  ...
  MtpNetworker.prototype.toggleOffline = function(enabled) {
    // dump('toggle ', enabled, this.dcID, this.iii);
  ...
  MtpNetworker.prototype.performSheduledRequest = function() {
    // dump(dT(), 'sheduled', this.dcID, this.iii);
    if (this.offline || akStopped) {
      dump(dT(), 'Cancel sheduled');
      return false;
    }
    delete this.nextReq;
    if (this.pendingAcks.length) {
      ...
      // dump('acking messages', ackMsgIDs);
    ...
    if (this.pendingResends.length) {
      ...
      // dump('resendReq messages', resendMsgIDs);
    ...
    if (messages.length > 1) {
      ...
      if (Config.Modes.debug) {
        dump(dT(), 'Container', innerMessages, message.msg_id, message.seq_no);
      }
    } else {
    ...
    this.sendEncryptedRequest(message).then(function (result) {
      self.toggleOffline(false);
      // dump('parse for', message);
      self.parseResponse(result.data).then(function (response) {
        if (Config.Modes.debug) {
          dump(dT(), 'Server response', self.dcID, response);
        }
      ...
      dump('Encrypted request failed', error);
  ...
  MtpNetworker.prototype.getEncryptedMessage = function (bytes) {
    ...
    // dump(dT(), 'Start encrypt', bytes.byteLength);
  ...
  MtpNetworker.prototype.sendEncryptedRequest = function (message, options) {
    var self = this;
    options = options || {};
    // dump(dT(), 'Send encrypted'/*, message*/);
    // console.trace();
    ...
    return this.getEncryptedMessage(data.getBuffer()).then(function (encryptedResult) {
      // dump(dT(), 'Got encrypted out message'/*, encryptedResult*/);
  ...
  MtpNetworker.prototype.parseResponse = function (responseBuffer) {
    // dump(dT(), 'Start parsing response');
    var self = this;
        ...
        var deserializerOptions = {
          ...
          override: {
            mt_message: function (result, field) {
              ...
              // dump(dT(), 'override message', result);
            },
            mt_rpc_result: function (result, field) {
              ...
              // dump(dT(), 'override rpc_result', sentMessage, type, result);
            }
          }
        };
  ...
  MtpNetworker.prototype.cleanupSent = function () {
    var self = this;
    var notEmpty = false;
    // dump('clean start', this.dcID/*, this.sentMessages*/);
    angular.forEach(this.sentMessages, function(message, msgID) {
      // dump('clean iter', msgID, message);
      if (message.notContentRelated && self.pendingMessages[msgID] === undefined) {
        // dump('clean notContentRelated', msgID);
        delete self.sentMessages[msgID];
      }
      else if (message.container) {
        for (var i = 0; i < message.inner.length; i++) {
          if (self.sentMessages[message.inner[i]] !== undefined) {
            // dump('clean failed, found', msgID, message.inner[i], self.sentMessages[message.inner[i]].seq_no);
            notEmpty = true;
            return;
          }
        }
        // dump('clean container', msgID);
        delete self.sentMessages[msgID];
      } else {
        notEmpty = true;
      }
  ...
  MtpNetworker.prototype.processMessage = function (message, messageID, sessionID) {
    // dump('process message', message, messageID, sessionID);
    switch (message._) {
      case 'msg_container':
      ...
      case 'bad_server_salt':
        dump(dT(), 'Bad server salt', message);
        var sentMessage = this.sentMessages[message.bad_msg_id];
        if (!sentMessage || sentMessage.seq_no != message.bad_msg_seqno) {
          dump(message.bad_msg_id, message.bad_msg_seqno);
      ...
      case 'bad_msg_notification':
        dump(dT(), 'Bad msg notification', message);
        var sentMessage = this.sentMessages[message.bad_msg_id];
        if (!sentMessage || sentMessage.seq_no != message.bad_msg_seqno) {
          dump(message.bad_msg_id, message.bad_msg_seqno);
      ...
      case 'rpc_result':
        ...
        if (sentMessage) {
          ...
          if (message.result._ == 'rpc_error') {
            ...
            if (deferred) {
              if (Config.Modes.debug) {
                dump(dT(), 'Rpc response', message.result);
              } else {
